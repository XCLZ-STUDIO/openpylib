...
PyThread_start_new_thread(void (*)(void *), void *);

PyAPI_FUNC(void)

_Py_NO_RETURN PyThread_exit_thread(void);
...
PyThread_free_lock(PyThread_type_lock);
...
PyThread_release_lock(PyThread_type_lock);
...
PyThread_set_key_value(int key,
                       void *value);
